...
function checkLinearMovement(events: BehaviorEvent[]): boolean {
...
function checkUniformSpeed(events: BehaviorEvent[]): boolean {
...
function checkRegularClicks(events: BehaviorEvent[]): boolean {
  if (events.length < 2) return false;
...
function isCollinear(p1: {x: number, y: number}, p2: {x: number, y: number}, p3: {x: number, y: number}): boolean {
  const area = Math.abs(
    (p2.x - p1.x) * (p3.y - p1.y) - 
    (p3.x - p1.x) * (p2.y - p1.y)
  );
